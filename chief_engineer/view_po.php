...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'approve') {
        $stmt = $conn->prepare("UPDATE po_awarding_letters SET chief_engineer_approval = 'approved', final_status = 'approved' WHERE id = ?");
        $stmt->bind_param("i", $po_id);
        $stmt->execute();
        $success = 'PO/Awarding letter approved successfully!';
        header('refresh:2;url=dashboard.php');
    } elseif ($action === 'reject') {
        $stmt = $conn->prepare("UPDATE po_awarding_letters SET chief_engineer_approval = 'rejected', final_status = 'rejected' WHERE id = ?");
        $stmt->bind_param("i", $po_id);
        $stmt->execute();
    ...
    <div class="container">
        <div class="dashboard-wrapper">
            ...
            <div class="dashboard-content">
                ...
                <?php if ($po['chief_engineer_approval'] === 'pending'): ?>
                <div class="card">
