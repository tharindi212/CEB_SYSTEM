...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'approve') {
        $stmt = $conn->prepare("UPDATE quotation_requests SET electrical_engineer_status = 'approved', electrical_engineer_date = NOW() WHERE id = ?");
        $stmt->bind_param("i", $request_id);

        if ($stmt->execute()) {
            $chief_engineers = getUsersByRole($conn, 'chief_engineer');

            $subject = "Quotation Request Approved by Electrical Engineer";
            $body = "<h3>Quotation Request Approved</h3>
                     <p>Request #{$request_id} has been approved by Electrical Engineer.</p>
        ...
        $stmt = $conn->prepare("UPDATE quotation_requests SET electrical_engineer_status = 'denied', electrical_engineer_reason = ?, electrical_engineer_date = NOW(), final_status = 'denied' WHERE id = ?");
        $stmt->bind_param("si", $reason, $request_id);

        if ($stmt->execute()) {
            $chief_engineers = getUsersByRole($conn, 'chief_engineer');

            $subject = "Quotation Request Denied by Electrical Engineer";
            $body = "<h3>Quotation Request Denied</h3>
                     <p>Request #{$request_id} has been denied by Electrical Engineer.</p>
    ...
    <div class="container">
        <div class="dashboard-wrapper">
            ...
            <div class="dashboard-content">
                ...
                <?php if ($request['electrical_engineer_status'] === 'pending'): ?>
                <div class="card">
