...
$stmt = $conn->prepare("SELECT qr.*, u.full_name as clerk_name, u.email as clerk_email FROM quotation_requests qr JOIN users u ON qr.clerk_id = u.id WHERE qr.id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'approve') {
        $stmt = $conn->prepare("UPDATE quotation_requests SET chief_clerk_status = 'approved', chief_clerk_date = NOW() WHERE id = ?");
        $stmt->bind_param("i", $request_id);
        ...
        if ($stmt->execute()) {
            ...
            $subject = "Quotation Request Approved by Chief Clerk";
            $body = "<h3>Quotation Request Approved</h3>
                     <p>Request #{$request_id} has been approved by Chief Clerk.</p>
        ...
        $stmt = $conn->prepare("UPDATE quotation_requests SET chief_clerk_status = 'denied', chief_clerk_reason = ?, chief_clerk_date = NOW(), final_status = 'denied' WHERE id = ?");
        $stmt->bind_param("si", $reason, $request_id);

        if ($stmt->execute()) {
            $chief_engineers = getUsersByRole($conn, 'chief_engineer');

            $subject = "Quotation Request Denied by Chief Clerk";
            $body = "<h3>Quotation Request Denied</h3>
                     <p>Request #{$request_id} has been denied by Chief Clerk.</p>
    ...
    <div class="container">
        <div class="dashboard-wrapper">
            ...
            <div class="dashboard-content">
                ...
                <?php if ($request['chief_clerk_status'] === 'pending'): ?>
                <div class="card">
